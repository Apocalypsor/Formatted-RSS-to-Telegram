...
class Config {
    constructor(input) {
        ...
        this.flaresolverr = input.flaresolverr || null;
        if (this.flaresolverr) {
            this.flaresolverr = this.flaresolverr.replace(/\/$/, "");
        }
    ...
    parseProxy(input) {
        ...
        for (const mustHaveKey of mustHave) {
            if (!(mustHaveKey in input)) {
                logger.error(
                    `Invalid Proxy config for ${input.name}, skipping!`
                );
                return null;
            }
        }
    ...
    parseTelegram(input) {
        ...
        for (const mustHaveKey of mustHave) {
            if (!(mustHaveKey in input)) {
                logger.error(
                    `Invalid Telegram config for ${input.name}, skipping!`
                );
                return;
            }
        }
