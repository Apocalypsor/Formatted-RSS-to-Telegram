...
export interface TaskMetadata {
    uniqueHash: string;
    textHash: string;
    url?: string;        // send only
    senderName?: string; // send only
    chatId?: bigint;     // send only
    historyId?: number;  // edit only
}
    ...
    private async execute(task: TaskWithDbId): Promise<void> {
        try {
            if (task.data.type === TASK_TYPE.SEND) {
                ...
                logger.debug(`Processing send task (DB ID: ${task.dbId}) for ${data.sender.name}`);
                ...
                if (data.metadata) {
                    try {
                        await finalizeHistory(data.metadata.uniqueHash, data.metadata.textHash, messageId);
                ...
                logger.debug(`Processing edit task (DB ID: ${task.dbId}) for ${data.sender.name}, message ${data.messageId}`);

                await edit(data.sender, messageIdBigInt, data.text);

                if (data.metadata) {
                    try {
                        await updateHistory(data.metadata.historyId!, data.metadata.textHash, messageIdBigInt);
            ...
            if (task.dbId) {
                await updateMessageStatus(task.dbId, QUEUE_STATUS.COMPLETED).catch((e) =>
            ...
            if (task.dbId) {
                await updateMessageStatus(task.dbId, QUEUE_STATUS.FAILED, errorDetail).catch((e) =>
                    logger.error(`Failed to update task status to failed (DB ID: ${task.dbId}): ${e}`),
                );
            }
    ...
    async recoverPendingTasks(): Promise<void> {
        ...
        for (const dbTask of pendingTasks) {
            try {
                ...
                void this.pQueue.add(() => this.execute({ data: taskData, dbId: dbTask.id }));
            } catch (error) {
