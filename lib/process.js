...
const {getSender, send, edit, notify} = require("./sender");
...
async function process(rssItem) {
    ...
    if (!sender) {
        logger.warn(`Sender ${rssItem.sendTo} for ${rssItem.name} not found, skipping.`);
        return;
    }
    ...
    if (rssContent.length === 0) {
        ...
        logger.warn(`RSS item ${rssItem.name} (${rssItem.url}) is expired for ${expireCount} times, skipping.`);
...
async function processItem(rssItem, sender, item) {
    ...
    tmpSender.disableNotification = rssItem.disableNotification || sender.disableNotification;
    tmpSender.disableWebPagePreview = rssItem.disableWebPagePreview || sender.disableWebPagePreview;
